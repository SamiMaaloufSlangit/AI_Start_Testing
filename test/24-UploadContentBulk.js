...
console.log('🚀 Starting Upload Content Bulk Test');
console.log('📋 Test Configuration:');

describe('Upload Content Bulk', function () {
    ...
    it('successfully upload content bulk', async function () {
        try {
            ...
            console.log('🔍 Looking for "Bulk Import" button');
            console.log('⏳ Waiting for "Bulk Import" button to appear');
            await driver.wait(until.elementLocated(By.xpath("//button[contains(@class, 'items-center') and contains(@class, 'justify-center') and contains(@class, 'whitespace-nowrap') and contains(@class, 'rounded-md') and contains(., 'Bulk Import')]")), 10000);
            console.log('  - "Bulk Import" button found');

            console.log('🔘 Clicking "Bulk Import" button');
            const bulkImportButton = await driver.findElement(By.xpath("//button[contains(@class, 'items-center') and contains(@class, 'justify-center') and contains(@class, 'whitespace-nowrap') and contains(@class, 'rounded-md') and contains(., 'Bulk Import')]"));
            await bulkImportButton.click();
            console.log('  - "Bulk Import" button clicked');

            await driver.sleep(2000);

            console.log('📁 Looking for CSV file upload input');
            console.log('⏳ Waiting for CSV file input to appear');
            await driver.wait(until.elementLocated(By.css("input#csvFile[type='file']")), 10000);
            console.log('  - CSV file input found');

            console.log('📤 Uploading course_content.csv file');
            const fileInput = await driver.findElement(By.css("input#csvFile[type='file']"));
            const csvFilePath = path.join(__dirname, '..', 'course_content.csv');
            console.log('  - CSV file path:', csvFilePath);

            await fileInput.sendKeys(csvFilePath);
            console.log('  - File uploaded successfully');

            await driver.sleep(2000);

            console.log('🔍 Looking for "Import Content" button');
            console.log('⏳ Waiting for "Import Content" button to appear');
            await driver.wait(until.elementLocated(By.xpath("//button[@type='submit' and contains(@class, 'bg-primary') and contains(@class, 'text-primary-foreground') and contains(., 'Import Content')]")), 10000);
            console.log('  - "Import Content" button found');

            console.log('🔘 Clicking "Import Content" button');
            const importContentButton = await driver.findElement(By.xpath("//button[@type='submit' and contains(@class, 'bg-primary') and contains(@class, 'text-primary-foreground') and contains(., 'Import Content')]"));
            await importContentButton.click();
            console.log('  - "Import Content" button clicked');

            console.log('⏳ Waiting for import notification');
            await driver.sleep(3000);
            console.log('  - Import notification received');

            console.log('🔍 Looking for "Save" button');
            console.log('⏳ Waiting for "Save" button to appear');
            await driver.wait(until.elementLocated(By.xpath("//button[contains(@class, 'bg-primary') and contains(@class, 'text-primary-foreground') and text()='Save']")), 10000);
            console.log('  - "Save" button found');

            console.log('🔘 Clicking "Save" button');
            const saveButton = await driver.findElement(By.xpath("//button[contains(@class, 'bg-primary') and contains(@class, 'text-primary-foreground') and text()='Save']"));
            await saveButton.click();
            console.log('  - "Save" button clicked');

            await driver.sleep(2000);

            console.log('✅ Upload Content Bulk test completed successfully');

        } catch (error) {
            console.error('❌ Upload Content Bulk test failed:', error.message);
            console.error('  - Error details:', error);
            throw error;
        }
    });
